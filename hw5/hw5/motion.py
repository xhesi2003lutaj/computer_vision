...
def iterative_lucas_kanade(img1, img2, keypoints,
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        if x1 - w < 0 or x1 + w >= img1.shape[1] or y1 - w < 0 or y1 + w >= img1.shape[0]:
            continue
        
        Ix_window = Ix[y1-w: y1+w+1, x1-w: x1+w+1].flatten()
        Iy_window = Iy[y1-w: y1+w+1, x1-w: x1+w+1].flatten()
        G = np.array([[np.sum(Ix_window**2), np.sum(Ix_window * Iy_window)],
                      [np.sum(Ix_window * Iy_window), np.sum(Iy_window**2)]])
        if np.linalg.det(G) < 1e-5:
        ...
        for k in range(num_iters):
            ...
            delta_I = img1[y1-w:y1+w+1, x1-w:x1+w+1] - img2[y2-w:y2+w+1, x2-w:x2+w+1]
            delta_I_flat = delta_I.flatten()
            b_k = np.array([np.sum(delta_I_flat * Ix_window),
                            np.sum(delta_I_flat * Iy_window)])
            vk = G_inv @ b_k
